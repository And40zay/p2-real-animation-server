...
static void cmd_create_rectangle(client_session_t* client, char* tokens[], int argc, char* response) {
    ...
    if (argc != 4) {  // command + width + height + color + filled
        snprintf(response, 256, "%d\n", RPC_VALUE_ERROR);
        return;
    }
    
    // Parse arguments: create_rectangle width height color filled
    size_t width = atoi(tokens[1]);
    size_t height = atoi(tokens[2]);
    color_t color = (color_t)atol(tokens[3]);
...
static void cmd_create_canvas(client_session_t* client, char* tokens[], int argc, char* response) {
    ...
    if (argc != 3) {  // command + height + width + background
        snprintf(response, 256, "%d\n", RPC_VALUE_ERROR);
        return;
    }
    ...
    color_t bg_color = (argc > 3) ? (color_t)atol(tokens[3]) : 0;
...
static void cmd_place_sprite(client_session_t* client, char* tokens[], int argc, char* response) {
    ...
    if (argc != 4) {  // command + canvas + sprite + x + y
        snprintf(response, 256, "%d\n", RPC_VALUE_ERROR);
        return;
    }
    
    struct canvas* canvas = (struct canvas*)atol(tokens[1]);
    struct sprite* sprite = (struct sprite*)atol(tokens[2]);
