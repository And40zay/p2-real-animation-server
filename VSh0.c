...
static struct sprite* load_bmp(const char* file) {
    ...
    int32_t raw_w = b5.bV5Width;
    int32_t raw_h = b5.bV5Height;
    if (raw_w <= 0 || raw_h == 0) { fclose(fp); return NULL; }
    bool top_down = raw_h < 0;
    size_t w = (size_t)raw_w;
    size_t h = (size_t)(top_down ? -(int64_t)raw_h : raw_h);
