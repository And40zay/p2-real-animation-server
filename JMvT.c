...
int main(void) {
    const char* paths[] = { "A.bmp", "../p1-scaffold/A.bmp", "/home/jake/Desktop/P1/p1-scaffold/A.bmp" };
    struct sprite* bmp = NULL;
    for (size_t i = 0; i < sizeof(paths)/sizeof(paths[0]); i++) {
        bmp = animate_create_sprite(paths[i]);
        if (bmp) {
            fprintf(stderr, "Loaded bitmap from: %s\n", paths[i]);
            break;
        }
    }
    if (!bmp) {
        fprintf(stderr, "Failed to load A.bmp, using generated rectangle instead.\n");
        bmp = animate_create_rectangle(30, 20, animate_color_argb(0xFF, 0, 128, 255), true);
    }

    size_t canvas_w = 200, canvas_h = 200;
    ...
    struct sprite* rect = animate_create_rectangle(40, 20, animate_color_argb(0xFF, 255, 255, 0), true);
