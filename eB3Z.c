...
// Decrement refcount only. Do not free here. Return true if refcount is 0.
static bool sprite_unref(struct sprite* s) {
    if (!s) return false;
    if ( s->ref_count > 0 ) {
        s->ref_count--;
    }
    return s->ref_count == 0;
}
...
static struct sprite* load_bmp(const char* file) {
    ...
    // Validate against file size to avoid huge allocations
    uint64_t need = (uint64_t)w * (uint64_t)h * sizeof(color_t);
    if (bmp_header.pixel_offset > bmp_header.file_size ||
        bmp_header.pixel_offset + need > bmp_header.file_size) {
        fclose(fp);
        return NULL;
    }
...
bool animate_destroy_sprite(struct sprite* sprite) 
{
    ...
    if (!sprite_unref(sprite)) return false; // still in use
    free(sprite->pixels);
    free(sprite);
    return true;
}
