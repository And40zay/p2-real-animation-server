#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <errno.h>

#define MAX_INPUT 512
#define MAX_RESPONSE 1024
#define FIFO_BASE "/tmp/animate_fifos"
#define WELL_KNOWN_FIFO FIFO_BASE "/server_fifo"

int main(int argc, char** argv, char** envp) {
    char input[MAX_INPUT];
    char response[MAX_RESPONSE];
    
    printf("Animation Client Started\n");
    printf("First, login with: Login <username>\n");
    printf("Then try commands like:\n");
    printf("  create_rectangle 10 20 0xFF0000 1\n");
    printf("  create_canvas 100 100 0\n");
    printf("  place_sprite <canvas_handle> <sprite_handle> 10 10\n");
    printf("Type 'quit' to exit\n\n");
    
    while (1) {
        printf("> ");
        fflush(stdout);
        
        if (fgets(input, sizeof(input), stdin) == NULL) {
            break;
        }
        
        // Remove newline
        input[strcspn(input, "\n")] = 0;
        
        if (strcmp(input, "quit") == 0) {
            break;
        }
        
        // Connect to server
        printf("Connecting to server...\n");
        int server_fd = open(WELL_KNOWN_FIFO, O_WRONLY);
        if (server_fd == -1) {
            printf("Failed to connect to server. Is it running? (errno: %d)\n", errno);
            continue;
        }
        
        // Send command
        printf("Sending: %s\n", input);
        write(server_fd, input, strlen(input));
        write(server_fd, "\n", 1);
        close(server_fd);
        
        // Now open FIFO for reading response (need a separate FIFO per client)
        char response_fifo[256];
        snprintf(response_fifo, sizeof(response_fifo), "%s/client_%d", FIFO_BASE, getpid());
        
        // Create response FIFO
        unlink(response_fifo);
        if (mkfifo(response_fifo, 0666) == -1) {
            printf("Failed to create response FIFO\n");
            continue;
        }
        
        // Open response FIFO for reading
        int resp_fd = open(response_fifo, O_RDONLY);
        if (resp_fd == -1) {
            printf("Failed to open response FIFO\n");
            unlink(response_fifo);
            continue;
        }
        
        // Read response
        ssize_t bytes_read = read(resp_fd, response, sizeof(response) - 1);
        if (bytes_read > 0) {
            response[bytes_read] = '\0';
            printf("%s", response);
        }
        
        close(resp_fd);
        unlink(response_fifo);
    }
    
    printf("Client exiting\n");
    return 0;
}