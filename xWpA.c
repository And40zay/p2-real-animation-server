...
static void sprite_dec(struct sprite* s) { if (s && s->ref_count > 0) s->ref_count--; }
...
static struct sprite* load_bmp(const char* file) {
    ...
    if (b5.bV5Width <= 0 || b5.bV5Height <= 0 || b5.bV5BitCount != 32) {
        fclose(fp);
        return NULL;
    }
    size_t w = (size_t)b5.bV5Width;
    size_t h = (size_t)b5.bV5Height;
    if (w == 0 || h == 0 || w > SIZE_MAX / h) { fclose(fp); return NULL; }
    ...
    uint64_t need = (uint64_t)w * (uint64_t)h * sizeof(color_t);
    if (bmp_header.pixel_offset < sizeof(bmp_header) ||
        (uint64_t)bmp_header.pixel_offset + need > (uint64_t)fsz) {
        fclose(fp);
        return NULL;
    }
...
bool animate_destroy_sprite(struct sprite* sprite) 
{
    ...
    if (sprite->ref_count != 0) return false; // still in use
    free(sprite->pixels);
    free(sprite);
    return true;
}
...
{
    if (!canvas) return;
    while (canvas->bottom_layer) {
        animate_destroy_placement(canvas->bottom_layer);
    }
    free(canvas);
}
...
void animate_destroy_placement(struct sprite_placement* sprite_placement)
{
    ...
    sprite_dec(sprite_placement->sprite);
