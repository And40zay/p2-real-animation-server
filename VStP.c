...
static void cmd_create_canvas(client_session_t* client, char* tokens[], int argc, char* response) {
    ...
    if (argc != 4) {
        snprintf(response, 256, "argc=%d %d\n", argc, RPC_VALUE_ERROR);
        return;
    }
    ...
    printf("Creating canvas: %ldx%ld bg=0x%06lx\n", height, width, bg_val);
...
static void cmd_place_sprite(client_session_t* client, char* tokens[], int argc, char* response) {
    ...
    unsigned long canvas_handle = strtoul(tokens[1], NULL, 10);
    unsigned long sprite_handle = strtoul(tokens[2], NULL, 10);
    ssize_t x = atol(tokens[3]);
    ssize_t y = atol(tokens[4]);
    
    printf("Placing sprite: canvas=%lu sprite=%lu at (%ld,%ld)\n", 
           canvas_handle, sprite_handle, x, y);
    
    struct canvas* canvas = (struct canvas*)canvas_handle;
    struct sprite* sprite = (struct sprite*)sprite_handle;
