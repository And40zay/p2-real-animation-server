...
static bool sprite_unref(struct sprite* s) {
    ...
    // Only free if this is the last reference (user's reference)
    if (s->ref_count == 1) {
        s->ref_count = 0;
        free(s->pixels);
        free(s);
    ...
    // Placements still exist, just decrement
    s->ref_count--;
    return false;
}
...
static struct sprite* load_bmp(const char* file) {
    ...
    // Prevent overflow/absurd allocations
    if (w == 0 || h == 0 || w > SIZE_MAX / sizeof(color_t) / h) { fclose(fp); return NULL; }
    size_t max_pixels = 0;
    if (bmp_header.file_size > bmp_header.pixel_offset)
        max_pixels = (bmp_header.file_size - bmp_header.pixel_offset) / sizeof(color_t);
    if (max_pixels && w * h > max_pixels) { fclose(fp); return NULL; }
