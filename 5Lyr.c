...
struct sprite* animate_create_sprite(const char* file) {
    ...
    if (fread(&bh, sizeof(bh), 1, fp) != 1 || bh.magic[0] != 'B' || bh.magic[1] != 'M') {
        fclose(fp);
        return NULL;
    }

    struct bitmapv5_header b5;
    if (fread(&b5, sizeof(b5), 1, fp) != 1) {
        fclose(fp);
        return NULL;
    }

    size_t w = b5.bV5Width;
    size_t h = b5.bV5Height;
    ...
    fseek(fp, bh.pixel_offset, SEEK_SET);
...
struct sprite* animate_create_circle(size_t radius, color_t c, bool filled) {
    ...
    for(size_t y = 0; y< diameter; y++)
    {
        for(size_t x = 0; x< diameter; x++)
        {
            if(filled)
            {
                s->pixels[y * diameter + x] = (c & 0x00FFFFFF) | 0xFF000000;
            }
            else
            {
...
struct sprite* animate_create_rectangle(size_t width, size_t height,
    ...
    for(size_t y = 0; y<height; y++)
    {
        for(size_t x = 0; x<width; x++)
        {
            if(filled)
            {
                s->pixels[y * width + x] = (c & 0x00FFFFFF) | 0xFF000000;
            }
            else
            {
                            ...
                            size_t frame_rate, void* buf) 
{
    ...
    {
        out[i] = (canvas->background_color & 0x00FFFFFF) | 0xFF000000;
    }
