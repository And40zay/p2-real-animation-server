...
static void sprite_deref(struct sprite* s) { if (s && s->ref_count > 0) s->ref_count--; }
...
static struct sprite* load_bmp(const char* file) {
    ...
    if (b5.bits_per_pixel != 32 || b5.bV5Width <= 0 || b5.bV5Height <= 0) {
        fclose(fp);
        return NULL;
    }
    size_t w = (size_t)b5.bV5Width;
    size_t h = (size_t)b5.bV5Height;
    if (w > SIZE_MAX / h || w * h > SIZE_MAX / sizeof(color_t)) {
        fclose(fp);
        return NULL;
    }
...
void animate_destroy_placement(struct sprite_placement* sprite_placement)
{
    ...
    sprite_deref(sprite_placement->sprite);
